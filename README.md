...
> ✅ **Upgraded to Spring AI 2.0.1 · Spring Boot 4.1.1 · Spring Cloud 2025.1.2.** Spring AI **2.0.0 → 2.0.1 is the first GA patch**; it ships eleven documented breaking changes, but **none of them touch this workshop** — zero application-code edits were required. The ones worth knowing about are behavioural defaults around tool calling: fallback tool resolution is now **off** by default, `DefaultToolCallingManager` enforces **call limits** (40 per tool, 150 total), `OpenAiChatModel` no longer forces `strict(true)` tool schemas, and `ToolCallingAdvisor` now **accumulates** token usage across the whole tool loop (Stage 8 traces will report higher totals). The workshop is unaffected because it always attaches tools explicitly via `.tools(...)` / `.toolCallbacks(...)`. Alongside it, Spring Boot moved **4.1.0 → 4.1.1** (Spring Framework 7.0.9, Spring Security 7.1.1, Micrometer 1.17.1, Tomcat 11.0.24) — a maintenance release whose only breaking change is Gradle-plugin-only, and Spring Cloud stays on **2025.1.2** (BOM range: Boot ≥ 4.0.0, < 4.2.0-M1). All 42 modules build green (`clean install`, all tests pass). Full breakdown: **[SPRING_AI_GA_TO_2_0_1_UPGRADE_PLAN.md](SPRING_AI_GA_TO_2_0_1_UPGRADE_PLAN.md)**. Prior records: **[SPRING_AI_RC1_TO_GA_UPGRADE_PLAN.md](SPRING_AI_RC1_TO_GA_UPGRADE_PLAN.md)**, **[SPRING_AI_M8_TO_RC1_UPGRADE_PLAN.md](SPRING_AI_M8_TO_RC1_UPGRADE_PLAN.md)**, **[SPRING_AI_M7_TO_M8_UPGRADE_PLAN.md](SPRING_AI_M7_TO_M8_UPGRADE_PLAN.md)**, **[SPRING_AI_M6_TO_M7_UPGRADE_PLAN.md](SPRING_AI_M6_TO_M7_UPGRADE_PLAN.md)**, **[SPRING_AI_M5_TO_M6_MIGRATION.md](SPRING_AI_M5_TO_M6_MIGRATION.md)**, **[SPRING_AI_M4_TO_M5_MIGRATION.md](SPRING_AI_M4_TO_M5_MIGRATION.md)**.
